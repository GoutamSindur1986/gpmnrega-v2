...
$(document).ready(function () {
    ...
    $("#ddldistrict").change(function () {
        ...
        callBlockAjax("/pullgpcodes?pull=block&code=" + $('option:selected', this).val());
    });

    $("#ddlblock").change(function () {

        callPanchayatAjax("/pullgpcodes?pull=panchayat&code=" + $('option:selected', this).val());

    });

    $('#txtPanchyatCode').blur(function () {

        callPanchayatAjax("/pullgpcodes?pull=panchayat&code=" + $(this).val());
    })
    ...
    $('#btnRegister').click(function (e) {
        ...
        if ($('#txtLokSabha').val().length < 2) {
            validate.push('lblLokSabha:Lok Sabha name cannot be empty');
        }

        if ($('#txtVidhanSabha').val().length < 2) {
            validate.push('lblVidhanSabha:Vidhan Sabha name cannot be empty');
        }
        ...
        callPanchayatAjax("/pullgpcodes?pull=panchayat&code=" + $('#txtPanchyatCode').val());
...
function callBlockAjax(_url) {
    $.ajax({
        ...
        success: function (response, textStatus, jqXHR) {
            $("#ddlblock").empty();
            $("#ddlblock").append('<option value="Select">Select Block</option>');
            $("#ddlblock").append($(response)[3].querySelectorAll('select')[1].innerHTML);
