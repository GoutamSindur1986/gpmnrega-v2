...
function loadStatsAndAutocomplete() {
    ...
    // ── 3. Load HH (Head of Household) mapping ──────────────────
    _loadHH();
}
// ── Load Head-of-Household mapping from NIC job card register ───
// Mirrors old gphome.js loadHH() — populates jobCardsHH[JCNO] = HH name
// Called once on page init so blank NMR rows have the correct HH column.
function _loadHH() {
    $.ajax({
        url: _API + 'jobcardhh',
        type: 'GET',
        ...
        success: function (resp) {
            try {
                ...
                if (tables[3]) {
                    tables[3].querySelectorAll('tr').forEach(function (row) {
                        var cells = row.cells;
                        if (cells && cells.length >= 3) {
                            var jcno = (cells[1].textContent || '').trim();
                            var hh   = (cells[2].textContent || '').trim();
                            if (jcno) jobCardsHH[jcno] = hh;
                        }
                    });
                    console.log('[GP Home] HH loaded:', Object.keys(jobCardsHH).length, 'entries');
                }
            } catch (e) {
                console.warn('[GP Home] _loadHH parse error:', e);
            }
        },
        error: function () {
            console.warn('[GP Home] _loadHH: failed to fetch HH data');
        }
    });
}
...
function _parseApplicantNameFromCellHtml(cellHtml) {
    try {
        ...
        var applicantname = name.indexOf(prefix) === 0 ? name.substr(prefix.length) :name;
    ...
    _ensureNmrHydrated(nmr, function (hydrated) {
        ...
        var tablerows = jcList.map(function (jc, i) {
            return {
                ...
                hh: (jobCardsHH && jobCardsHH[jc.JCNO]) ? jobCardsHH[jc.JCNO] : (jc.appName || ''),
                Place: jc.appAddress || '',
                bkname: jc.BankName || '',
                bkacc: 'XXXXXXXXXXXX',
        ...
        gpShowLoading('Generating Blank NMR…');
        $.ajax({
            url: '/api/proxy/blknmrpdf',
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({ accdata: tablerows, meta: meta }),
            xhrFields: { responseType: 'blob' },
            success: function (blob, status, xhr) {
                try {
                    var filename = (meta.workcode || 'blanknmr') + '_' + Date.now() + '.pdf';
                    ...
                    gpHideLoading();
                    gpToast('Error saving Blank NMR: ' + e.message, 'error');
                }
            },
            error: function (xhr) {
                gpHideLoading();
                gpToast('Blank NMR generation failed: ' + (xhr.responseJSON && xhr.responseJSON.error ? xhr.responseJSON.error : xhr.statusText || 'network error'), 'error');
            }
        });
    });
};
